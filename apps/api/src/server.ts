...
// import { v4 as uuidv4 } from 'uuid'; // No longer needed for SSE client IDs
// import { ProgressUpdate } from '../../packages/types/src'; // No longer needed for SSE
...
// subscriber.on('connect', () => console.log('Redis subscriber connected for SSE')); // Clarified log
// subscriber.on('error', (err: Error) => console.error('Redis SSE subscriber error:', err)); // Clarified log
//
// subscriber.on('message', (channel: string, message: string) => {
// console.log(\`Received message from ${channel} for SSE: ${message}\`); // Clarified log
...
// const jobClients = clients.get(jobId);
// if (jobClients) {
// console.log(\`Sending SSE update to ${jobClients.length} clients for job ${jobId}\`); // Clarified log
...
// console.log(\`SSE Client ${clientId} connected for job ${jobId}. Total clients for job: ${clients.get(jobId)?.length}\`); // Clarified log
//
...
// if (err) {
// console.error(\`Failed to subscribe to ${channelName} for SSE:`, err); // Clarified log
// return res.status(500).end();
// }
// console.log(\`Subscribed to ${channelName} for SSE. Total subscriptions: ${count}\`); // Clarified log
...
// req.on('close', () => {
// console.log(\`SSE Client ${clientId} disconnected for job ${jobId}\`); // Clarified log
// const jobClients = clients.get(jobId);
// if (jobClients) {
// const index = jobClients.findIndex(c => c.id === clientId);
// if (index !== -1) {
// jobClients.splice(index, 1);
// console.log(\`Removed SSE client ${clientId}. Remaining clients for job ${jobId}: ${jobClients.length}\`); // Clarified log
// // If no clients left for this job, unsubscribe from Redis channel
// if (jobClients.length === 0) {
// console.log(\`No SSE clients left for job ${jobId}. Unsubscribing from ${channelName}...\`); // Clarified log
